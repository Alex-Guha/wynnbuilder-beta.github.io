...
if (typeof window !== 'undefined') {
    ...
    const projectBase = pathPrefix ? '/' + pathPrefix[1] : '';
    const previewPrefix = window.location.pathname.match(new RegExp('^' + projectBase.replace(/\./g, '\\.') + '(\/_preview\/[^/]+)\/'));
    SITE_BASE = projectBase + (previewPrefix ? previewPrefix[1] : '');
...
const isMobile = (typeof navigator !== 'undefined') ? function() {
    let check = false;
    ...
    return check;
}() : false; // runs immediately, so isMobile is a boolean not a function
...
function setValue(id, value) {
    ...
    if (el == null) {
        console.warn("Tried to set text value of id {"+id+"} to ["+value+"] but did not exist!");
        return;
    }
    el.value = value;
...
let scrollPos = 0
if (typeof document !== 'undefined' && typeof screen !== 'undefined' && screen.width < 992) {
