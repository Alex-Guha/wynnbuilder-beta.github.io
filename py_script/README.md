...
- Copy the generated files into cutting edge `data/baseline/compressed/*compress.json`, and versioned `data/<ver>/`.
...
- Update aspects, majid, atree (in `data/baseline/`) to make sure they work.
- run `py_script/atree-generateID.py` in `py_script/` to "compile/link" and minify majid, atree, aspects. Copy result files `*_min/clean.json` into `data/<ver>` and rename according to previous versions.
- run `py_script/research/plot_dps.py` to generate dps data (outputs to `data/baseline/dps_data.json`), copy into `data/<ver>`.
